...
To get started with the CSS variables exported, you must have the contents of `build/css/variables.css` included in your html page. Perhaps directly through a `<link rel="stylesheet">` element, or indirectly through webpack's css-loader and `import '@QorAI/qora/build/css/variables.css'`.
...
Any color in QorAI's standard or extended palettes is available in a dark and light versions:
...
A tailwind config, complete with plugin, is available at `@QorAI/qora/build/tailwind/index.js`. Once you configure this, all variables should be available using the `theme()` function.
