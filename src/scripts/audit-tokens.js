...
const tokenRegex = /--qora-([a-zA-Z0-9]|-)+/gi
...
/**
 * Extracts all Qora tokens from a piece of text
 * @param {string} text
 * @returns {string[]} All the Qora CSS variables in the source text. Not deduplicated
 */
...
/**
 * Extracts all Qora tokens from a file
...
/**
 * Returns a set of all available Qora tokens
...
/**
 * Checks a folder to see if any files in it reference non-existent Qora tokens
 * @param {string} folder The folder to check for unknown Qora tokens
 */
...
${usages.map((u) => `    ${u}`).join('\n')}`
    ...
    console.error(
      'The above tokens are not present in Qora, and may have been used by mistake.'
    )
    process.exit(1)
  }
  console.log('Success!')
