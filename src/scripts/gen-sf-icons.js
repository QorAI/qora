 ...
 * dependencies...
 *
 * As a workaround, we've created the qora-sf-symbols repository which contains
 * prebuilt SF Symbols. The script is include in Qora so you can **choose** to
 * use it to build them yourself, and to make the qora-sf-symbols repository as
 * dumb as possible.
 */
  ...
  .then(async (icons) => {
    ...
    icons
      ...
      .map(async (icon) => {
        ...
        const outputFileName = `qora.${icon.replace('-', '.')}`
        await fs.writeFile(
