...
<div
  class="qora-progressring"
  class:spin={mode === 'indeterminate'}
  style:--progress={progress}
>
  <slot />
  <svg viewBox="0 0 48 48">
    <circle class="qora-progressring__background" />
    <circle class="qora-progressring__ring" />
  </svg>
</div>
...
<style lang="scss">
  ...
  .qora-progressring {
    --size: var(--qora-progressring-size, 48px);
    --spin-rate: var(--qora-progressring-spin-rate, 1s);
    --transition-duration: var(--qora-progressring-transition-duration, 0.2s);
    --stroke-color: var(
      --qora-progressring-color,
      var(--qora-color-button-background)
    );
    --stroke-width: var(--qora-progressring-stroke-width, 4px);
    --radius: 24px;
