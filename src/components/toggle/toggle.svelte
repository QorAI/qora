...
<label class={`qora-toggle size-${size}`}>
  <button
    on:mousedown={(e) => {
...
<style lang="scss">
  ...
  .qora-toggle {
    ...
    --duration: var(--qora-toggle-transition-duration, 0.12s);
    @media (prefers-reduced-motion) {
      --duration: 0;
    }

    --icon-size: var(--qora-icon-size, 20px);
    --width: var(--qora-toggle-width, 52px);
    --height: var(--qora-toggle-height, 32px);
    --padding: var(--qora-toggle-padding, var(--qora-spacing-s));
    --checked-color: var(
      --qora-toggle-checked-color,
      var(--qora-color-schemes-primary)
    );
    --unchecked-color: var(
      --qora-toggle-unchecked-color,
      var(--qora-color-schemes-outline-variant)
    );
    --thumb-color: var(
      --qora-toggle-thumb-color,
      var(--qora-color-schemes-on-primary)
    );
    --thumb-disabled-color: var(--qora-toggle-thumb-disabled-color, white);
    --label-gap: var(--qora-toggle-label-gap, var(--qora-spacing-s));
    --label-flex-direction: var(--qora-toggle-label-flex-direction, row);

    &.size-small {
      --width: 40px;
      --height: 24px;
      --icon-size: var(--qora-icon-size, 12px);
    }
  ...
  .qora-toggle button {
    ...
    &:focus-visible {
      box-shadow: var(--qora-effect-focus-state-offset);
    }
    ...
    &[aria-checked="false"] .thumb {
      --thumb-color: var(--qora-color-white);
    }
