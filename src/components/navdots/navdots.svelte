...
<nav class="qora-navdots" aria-label={label}>
...
<style lang="scss">
  ...
  .qora-navdots {
    --dot-size: var(--qora-navdots-size, 8px);
    // Expanded dot grows to fill half spacing in each direction.
    --expanded-dot-size: var(
      --qora-navdots-expanded-size,
      calc(var(--dot-size) + var(--dot-spacing))
    );

    --dot-spacing: var(--qora-navdots-spacing, 10px);
    --dot-vertical-margin: var(--qora-navdots-vertical-margin, 1px);
    --transition-duration: var(--qora-navdots-transition-duration, 0.2s);
    --transition-easing: var(--qora-navdots-easing, ease-in-out);

    --active-dot-color: var(
      --qora-navdots-active-color,
      var(--qora-color-button-background)
    );
    --active-dot-color-hover: var(
      --qora-navdots-active-color-hover,
      var(--qora-color-icon-interactive)
    );
    --dot-color: var(--qora-navdots-color, var(--qora-color-primary-20));
    --dot-color-hover: var(
      --qora-navdots-color-hover,
      var(--qora-color-primary-30)
    );
      ...
      transform: translate(
        calc(
          ((var(--dot-size) + var(--dot-spacing)) * var(--current-dot) -
            var(--dot-spacing) / 2) * var(--qora-direction)
        ),
        0
      );
