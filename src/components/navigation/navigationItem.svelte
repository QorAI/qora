...
<style lang="scss">
  :global(qora-navigationitem[data-selected='true']),
  .qora-navigation-item[data-selected='true'] {
  ...
  .qora-navigation-item {
    --nav-item-color: var(--qora-color-text-secondary);
    --qora-icon-color: var(--qora-color-icon-default);

    position: relative;

    // When this item is selected, set it as the active indicator
    &[data-selected='true'] {
      --nav-item-color: var(--qora-color-text-interactive);
    ...
    button {
      ...
      align-items: center;
      height: 48px;
      padding-left: var(--qora-spacing-2xl);
      padding-right: var(--qora-spacing-m);
      border-radius: 0;
      outline: none;
